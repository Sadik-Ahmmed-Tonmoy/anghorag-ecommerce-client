...
const CardForOfferPage = ({ item, itemNumber }) => {
        ...
        <>
            <div className="relative bg-white">
                ...
                <div className=" card min-w-min card-compact p-3 border-[0.5px] border-[#ff87a680] rounded-lg  hover:shadow-xl mb-3 sm:mb-6 md:mb-9">
                    <Link to={`/product/${item?.product_info?.slug}/${item.campaign_id}`}>
                        <div className="mb-[14px] w-full mx-auto rounded-lg hover:cursor-pointer">
                            <ImageURL
                                className="w-full xs:h-[210px] object-fill rounded-lg shrink-0"
                                image={item?.product_info?.main_images && item?.product_info?.main_images}
                            />
                        </div>
                    </Link>
                        ...
                        <Link to={`/product/${item?.product_info?.slug}/${item.campaign_id}`}>
                            <h2 className="font-semibold h-10 text-black text-base mb-[9px] leading-[22px] tracking-[-0.16px] hover:cursor-pointer">
                                {item?.product_info?.name &&
                                    (item.product_info.name.length > 40 ? `${item.product_info.name.slice(0, 40)}...` : item.product_info.name)}
                            </h2>
                        </Link>
                        <div className="flex items-center gap-3 mb-[14px]">
                            <StarRating rating={item?.product_info?.review_avg_star} totalStars={5} size={16} />
                            <span className="text-black text-sm font-medium tracking-[-0.14px]">({item?.product_info?.review_count})</span>
                        </div>
                        <div className="flex items-center flex-wrap gap-2">
                            <p className="text-black text-lg font-semibold flex items-center tracking-[-0.72px]">
                                <TbCurrencyTaka size={21} />
                                {item?.flat_discount ? item.current_price : item?.product_info?.price}
                            </p>
                            ...
                            {item?.percent_discount === null ? (
                                ""
                            ) : (
                                <>
                                    {item?.percent_discount && item?.percent_discount > 0 && (
                                        <p className="text-black text-xs font-medium tracking-[-0.48px]">(-{parseInt(item?.percent_discount)}%)</p>
                                    )}
                                </>
                            )}
                        </div>
                            ...
                            <button
                                onClick={() =>
                                    handleAddToCart(
                                        ...
                                        item?.product_info?.main_images,
                                        item.product_info.name,
                                        item?.product_info?.price,
                                        item.current_price,
                                        item?.product_info?.price * counter,
                                         item.current_price ? (item?.product_info?.price - item.current_price) * counter : 0,
                                        item?.product_info?.weight,
                                        item?.product_info?.weight * counter,
...
export default CardForOfferPage;
